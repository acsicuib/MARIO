...
%membrane(adapt,Si,TotalRR,NewSiFlavour) :-
%    serviceInstance(Si, S, (Id_F,_,_), self), service(S,SVersions,_),
%    %node(self, AvailableHW, _),
%    member(NewSiFlavour, SVersions), NewSiFlavour=(Id_F2,_,MRR_F2), dif(Id_F2,Id_F), MRR_F2 >= TotalRR,
%    \+ (member((_,_,MRR_F3), SVersions),  MRR_F3 >= TotalRR).
