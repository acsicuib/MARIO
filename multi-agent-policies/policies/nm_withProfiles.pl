...
n_operation(shrinkNewComerANDaccept,(Op,Si,SiFlavour),(Si,F2,_)) :-
    ...
    operator(_, Services, free), member(Si, Services), %SF: new for profiles
    member((F2,HW_F2,_),SVersions), node(self, AvailableHW, _), AvailableHW >= HW_F2, 
    \+ (member((F3,HW_F3,_), SVersions), dif(F2,F3), AvailableHW >= HW_F3, HW_F3 >= HW_F2).

%-make shrinkANDaccept and evictANDaccept more sophisticated (trying to shrink/evict not just the least prodctive)


%//for Stefano
% POSSIBILITIES for nm2 with paying users
%-before shrinking or evicting a bacterion currently running in self, we shrink the incoming bacterion if possible and if not a subcriber
%-shrinkANDaccept and evictANDaccept only if new bacteria is of a subscriber
%-make shrinkANDaccept and evictANDaccept more sophisticated (trying to shrink/evict not just the least prodctive)
...
n_operation(shrinkANDaccept,(Op,Si,SiFlavour),(Sj,SjFlavour,NewSjFlavour)) :-
    operation(Op,Si,SiFlavour), (Op=migrate; Op=replicate),
    operator(_, Services, subscriber), member(Si, Services), %SF: new for profiles
    SiFlavour=(_,HW_F,_),
    node(self, AvailableHW, _),
    NeededHW is HW_F - AvailableHW,            
...
modifiableInstances(SortedL) :-
    findall((P,S,SFlavour), (productivity(S,SFlavour,P), operator(_, Services, free), member(Sj, Services)),L), 
    msort(L,SortedL).
